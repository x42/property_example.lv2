...
###############################################################################
propex_VERSION?=$(shell git describe --tags HEAD 2>/dev/null | sed 's/-g.*$$//;s/^v//' || echo "LV2")
LIB_EXT=.so

LV2DIR ?= $(PREFIX)/$(LIBDIR)/lv2
LV2NAME=property_example
BUNDLE=property_example.lv2
BUILDDIR=build/
